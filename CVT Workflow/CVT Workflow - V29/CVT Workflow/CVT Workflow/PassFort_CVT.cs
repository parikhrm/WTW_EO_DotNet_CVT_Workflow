...
namespace CVT_Workflow
{
    public partial class PassFort_CVT : Form
    {
        ...
        public void reset_overall()
        {
            ...
            query_resolved_time.CustomFormat = " ";
            checkBox3.Checked = false;
            volumes.Text = string.Empty;
            approved_date.CustomFormat = " ";
            approved_time.CustomFormat = " ";
            approved_by.SelectedIndex = -1;
            pf_risk_category.SelectedIndex = -1;
            public_private.SelectedIndex = -1;
            pf_provided_director.SelectedIndex = -1;
            pf_provided_shareholders.SelectedIndex = -1;
            pf_provided_ubo.SelectedIndex = -1;
            no_of_dno_ubo_sh_added.Text = string.Empty;
            ubo_identified_by.SelectedIndex = -1;
            ubo_search_results.Text = string.Empty;
            dnb_report_used.SelectedIndex = -1;
        ...
        private void ubo_validation_time_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape || e.KeyCode == Keys.Delete || e.KeyCode == Keys.Space || e.KeyCode == Keys.Back)
            {
                ubo_validation_time.CustomFormat = " ";
        ...
        private void ubo_validation_time_MouseDown(object sender, MouseEventArgs e)
        {
            ubo_validation_time.CustomFormat = "HH:mm:ss";
            ubo_validation_time.Text = DateTime.Now.ToLongTimeString();
        }
        ...
        private void query_resolved_time_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape || e.KeyCode == Keys.Delete || e.KeyCode == Keys.Space || e.KeyCode == Keys.Back)
            {
                query_resolved_time.CustomFormat = " ";
        ...
        private void query_resolved_time_MouseDown(object sender, MouseEventArgs e)
        {
            query_resolved_time.CustomFormat = "HH:mm:ss";
            query_resolved_time.Text = DateTime.Now.ToLongTimeString();
        }
        ...
        private void approved_time_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape || e.KeyCode == Keys.Delete || e.KeyCode == Keys.Space || e.KeyCode == Keys.Back)
            {
                approved_time.CustomFormat = " ";
        ...
        private void approved_time_MouseDown(object sender, MouseEventArgs e)
        {
            approved_time.CustomFormat = "HH:mm:ss";
            approved_time.Text = DateTime.Now.ToLongTimeString();
        }
        ...
        public void datagridview_display_overall()
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(searchby_partyname.Text) && string.IsNullOrEmpty(searchby_orgid.Text))
                {
                    ...
                    cmd.CommandText = "select top 100 RequestID,PartyName,Received_Date,Received_Time,Requestor_Email_Address,Legal_Entity_Name,Role,OrgID,Org_Status,Inflow_Processed_By,Inflow_Processed_Date,Inflow_Processed_Time,UBO_Validated_By,UBO_Validated_Date,UBO_Validated_Time,Query_Raised_Date,Query_Raised_Time,Query_Category,Queried_For,Query_Raised_By,Query_Resolved_Date,Query_Resolved_Time,Volumes,Approved_Date,Approved_Time,Approved_By,PF_Risk_Category,Public_Private,PF_Provided_Directors,PF_Provided_UBO,PF_Provided_Shareholders,No_Of_DNO_UBO_SH_Added,UBO_Identified_By,UBO_Search_Results,DNB_Report_Used,Other_Parties,GCID_In_ECS_Status,FCA_Updated_In_Eclipse,Comments,Sub_Categories_QC,Categories_QC,QC_Date,QC_Done_By,a.LastUpdatedDateTime,b.EmpName,Party_Location,New_Client,PF_Log from dbo.tbl_passfort_cvt_daily_dotnet a with(nolock)  inner join dbo.tbl_emp_details b with(nolock) on a.lastupdatedby = substring(b.intid,5,len(b.intid)) where a.IsDeleted = 0 and a.lastupdatedby = @lastupdatedby";
        ...
        private void checkBox3_CheckedChanged(object sender, EventArgs e)
        {
            if (checkBox3.Checked == true)
            {
                ...
                query_raised_time.Enabled = true;
        ...
        private void insert_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@Inflow_Processed_Time",inflow_processed_time.Value.ToLongTimeString());
                ...
                if (ubo_validation_date.Text.Trim() == string.Empty)
                {
                    ...
                    cmd.Parameters.AddWithValue("@UBO_Validated_Time", DBNull.Value);
                }
                else
                {
                    cmd.Parameters.AddWithValue("@UBO_Validated_Date", ubo_validation_date.Value.Date);
                    cmd.Parameters.AddWithValue("@UBO_Validated_Time", ubo_validation_time.Value.ToLongTimeString());
                }
                ...
                if (query_resolved_date.Text.Trim() == string.Empty)
                {
                    ...
                    cmd.Parameters.AddWithValue("@Query_Resolved_Time", DBNull.Value);
                }
                else
                {
                    cmd.Parameters.AddWithValue("@Query_Resolved_Date", query_resolved_date.Value.Date);
                    cmd.Parameters.AddWithValue("@Query_Resolved_Time", query_resolved_time.Value.ToLongTimeString());
                }
                ...
                if (string.IsNullOrEmpty(approved_by.Text))
                {
                    ...
                    cmd.Parameters.AddWithValue("@Approved_Time", DBNull.Value);
                    cmd.Parameters.AddWithValue("@Approved_By", DBNull.Value);
                }
                else
                {
                    cmd.Parameters.AddWithValue("@Approved_Date", approved_date.Value.Date);
                    cmd.Parameters.AddWithValue("@Approved_Time", approved_time.Value.ToLongTimeString());
                ...
                if (string.IsNullOrEmpty(pf_provided_director.Text))
                {
                    cmd.Parameters.AddWithValue("@PF_Provided_Directors", DBNull.Value);
                }
                else
                {
                    cmd.Parameters.AddWithValue("@PF_Provided_Directors", pf_provided_director.Text);
                }
                if (string.IsNullOrEmpty(pf_provided_ubo.Text))
                {
                    cmd.Parameters.AddWithValue("@PF_Provided_UBO", DBNull.Value);
                }
                else
                {
                    cmd.Parameters.AddWithValue("@PF_Provided_UBO", pf_provided_ubo.Text);
                }
                if (string.IsNullOrEmpty(pf_provided_shareholders.Text))
                {
                    cmd.Parameters.AddWithValue("@PF_Provided_Shareholders", DBNull.Value);
                }
                else
                {
                    cmd.Parameters.AddWithValue("@PF_Provided_Shareholders", pf_provided_shareholders.Text);
                }
                ...
                if (string.IsNullOrEmpty(ubo_identified_by.Text))
                {
                    cmd.Parameters.AddWithValue("@UBO_Identified_By", DBNull.Value);
                }
                else
                {
                    cmd.Parameters.AddWithValue("@UBO_Identified_By", ubo_identified_by.Text);
                }
                if (string.IsNullOrEmpty(ubo_search_results.Text))
                {
                    cmd.Parameters.AddWithValue("@UBO_Search_Results", DBNull.Value);
                }
                else
                {
                    cmd.Parameters.AddWithValue("@UBO_Search_Results", ubo_search_results.Text);
                }
                if (string.IsNullOrEmpty(dnb_report_used.Text))
                {
                    cmd.Parameters.AddWithValue("@DNB_Report_Used", DBNull.Value);
                }
                else
                {
                    cmd.Parameters.AddWithValue("@DNB_Report_Used", dnb_report_used.Text);
                }
                ...
                else if (inflow_processed_time.Text.Trim() == string.Empty)
                {
                    MessageBox.Show("Please update Inflow processed time");
                }
                ...
                else if (string.IsNullOrEmpty(public_private.Text) && approved_date.Text.Trim() != string.Empty)
                {
                    MessageBox.Show("Please update Public Private");
                }
                ...
                else if (checkBox3.Checked && query_raised_time.Text.Trim() == string.Empty)
                {
                ...
                else if (approved_date.Text.Trim() != string.Empty && approved_time.Text.Trim() == string.Empty)
                {
                ...
                else if (query_resolved_date.Text.Trim() != string.Empty && query_resolved_time.Text.Trim() == string.Empty)
                {
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                if (e.RowIndex >= 0)
                {
                    ...
                    inflow_processed_time.Text = row.Cells["txt_Inflow_Processed_Time"].Value.ToString();
                    inflow_processed_time.CustomFormat = "HH:mm:ss";
                    ...
                    else
                    {
                        ...
                        ubo_validation_date.CustomFormat = "dd-MMMM-yyyy";
                        ubo_validation_time.Text = row.Cells["txt_UBO_Validated_Time"].Value.ToString();
                        ubo_validation_time.CustomFormat = "HH:mm:ss";
                    }
                    ...
                    else
                    {
                        ...
                        query_resolved_date.CustomFormat = "dd-MMMM-yyyy";
                        query_resolved_time.Text = row.Cells["txt_Query_Resolved_Time"].Value.ToString();
                        query_resolved_time.CustomFormat = "HH:mm:ss";
                    }
                    ...
                    else
                    {
                        ...
                        approved_date.CustomFormat = "dd-MMMM-yyyy";
                        approved_time.Text = row.Cells["txt_Approved_Time"].Value.ToString();
                        approved_time.CustomFormat = "HH:mm:ss";
                    ...
                    if (string.IsNullOrEmpty(row.Cells["txt_PF_Provided_Directors"].Value.ToString()))
                    {
                        pf_provided_director.SelectedIndex = -1;
                    }
                    else
                    {
                        pf_provided_director.Text = row.Cells["txt_PF_Provided_Directors"].Value.ToString();
                    }
                    if (string.IsNullOrEmpty(row.Cells["txt_PF_Provided_UBO"].Value.ToString()))
                    {
                        pf_provided_ubo.SelectedIndex = -1;
                    }
                    else
                    {
                        pf_provided_ubo.Text = row.Cells["txt_PF_Provided_UBO"].Value.ToString();
                    }
                    if (string.IsNullOrEmpty(row.Cells["txt_PF_Provided_Shareholders"].Value.ToString()))
                    {
                        pf_provided_shareholders.SelectedIndex = -1;
                    }
                    else
                    {
                        pf_provided_shareholders.Text = row.Cells["txt_PF_Provided_Shareholders"].Value.ToString();
                    } 
                    ...
                    if (string.IsNullOrEmpty(row.Cells["txt_UBO_Identified_By"].Value.ToString()))
                    {
                        ubo_identified_by.SelectedIndex = -1;
                    }
                    else
                    {
                        ubo_identified_by.Text = row.Cells["txt_UBO_Identified_By"].Value.ToString();
                    }
                    if (string.IsNullOrEmpty(row.Cells["txt_UBO_Search_Results"].Value.ToString()))
                    {
                        ubo_search_results.SelectedIndex = -1;
                    }
                    else
                    {
                        ubo_search_results.Text = row.Cells["txt_UBO_Search_Results"].Value.ToString();
                    }
                    if (string.IsNullOrEmpty(row.Cells["txt_DNB_Report_Used"].Value.ToString()))
                    {
                        dnb_report_used.SelectedIndex = -1;
                    }
                    else
                    {
                        dnb_report_used.Text = row.Cells["txt_DNB_Report_Used"].Value.ToString();
                    }
        ...
        private void inflow_processed_time_MouseDown(object sender, MouseEventArgs e)
        {
            inflow_processed_time.CustomFormat = "HH:mm:ss";
            inflow_processed_time.Text = DateTime.Now.ToLongTimeString();
        }

        private void inflow_processed_time_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape || e.KeyCode == Keys.Delete || e.KeyCode == Keys.Space || e.KeyCode == Keys.Back)
            {
                inflow_processed_time.CustomFormat = " ";
        ...
        private void pf_provided_director_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape || e.KeyCode == Keys.Delete || e.KeyCode == Keys.Space || e.KeyCode == Keys.Back)
            {
                pf_provided_director.SelectedIndex = -1;
        ...
        private void pf_provided_ubo_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape || e.KeyCode == Keys.Delete || e.KeyCode == Keys.Space || e.KeyCode == Keys.Back)
            {
                pf_provided_ubo.SelectedIndex = -1;
        ...
        private void pf_provided_shareholders_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape || e.KeyCode == Keys.Delete || e.KeyCode == Keys.Space || e.KeyCode == Keys.Back)
            {
                pf_provided_shareholders.SelectedIndex = -1;
        ...
        private void ubo_identified_by_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape || e.KeyCode == Keys.Delete || e.KeyCode == Keys.Space || e.KeyCode == Keys.Back)
            {
                ubo_identified_by.SelectedIndex = -1;
        ...
        private void ubo_search_results_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape || e.KeyCode == Keys.Delete || e.KeyCode == Keys.Space || e.KeyCode == Keys.Back)
            {
                ubo_search_results.SelectedIndex = -1;
        ...
        private void dnb_report_used_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape || e.KeyCode == Keys.Delete || e.KeyCode == Keys.Space || e.KeyCode == Keys.Back)
            {
                dnb_report_used.SelectedIndex = -1;
